...
/* Le passage d'un aimant provoque la mise à jour des compteurs
 *  speed_1_cnt ou speed_2_cnt, selon la roue
 *  pour mesurer la vitesse, on peut remettre ces compteurs à zéro au début de l'intervalle de temps
 *  la remise a zéro doit être faite immédiatement après la copie de ces variables
 */
...
volatile int speed_1_cnt=0;
volatile int speed_2_cnt=0;
/*******************************************************************************
 * Code
 ******************************************************************************/
int Speed_Get_1(void)
{
 int v= speed_1_cnt;
 speed_1_cnt= 0;
 return v;
}
int Speed_Get_2(void)
{
 int v= speed_2_cnt;
 speed_2_cnt= 0;
 return v;
 ...
 * Obsolète
  OVF Overflow
  C counter value
 Mesure du temps entre deux passages OVF contient le nombre de fois que le timer a débordé
delta=speed_1_C-speed_1_C_o+(speed_1_OVF-speed_1_OVF_o)*0xFFFF;
PRINTF("Speed (1) %08d %08d %08d %08d %08d %08d\r\n",
		delta, speed_1_C, speed_1_OVF, speed_1_C_o, speed_1_OVF_o,		s1n);
delta=speed_2_C-speed_2_C_o+(speed_2_OVF-speed_2_OVF_o)*0xFFFF;
PRINTF("     Speed (2) %08d %08d %08d %08d %08d %08d\r\n",
		delta, speed_2_C, speed_2_OVF, speed_2_C_o, speed_2_OVF_o, s2n);
