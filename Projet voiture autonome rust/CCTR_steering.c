...
* @param direction signed relative direction to the offset
* @return the effective direction after saturation to +/- STEERING_RELATIV_AMPLITUDE
**/
int16_t Steering_UpdatePwm_Relative(int16_t direction) {
	/* Updated duty cycle */
	 int16_t direction_actual=  direction ;
	 if (direction_actual <-STEERING_RELATIVE_AMPLITUDE ) direction_actual= -STEERING_RELATIVE_AMPLITUDE ;
	 if (direction_actual > STEERING_RELATIVE_AMPLITUDE ) direction_actual= STEERING_RELATIVE_AMPLITUDE ;
	 uint16_t direction_pwm=  STEERING_RELATIVE_OFFSET + direction_actual;
	TPM_UpdatePwmDutycycle_10000(STEERING_TIMER_BASEADDR,
			(tpm_chnl_t) STEERING_CHANNEL, kTPM_EdgeAlignedPwm, direction_pwm);
	//PRINTF("SA %d\n\r", direction_c);
	return direction_actual;
}

/**
* Update the duty cycle of the steering pwm signal around the zero position, NOT saturated
* DO NOT USE AS AN APPLICATION => risk to destroy the servomotor
* Returns the effective steering
* @param direction signed relative direction to the offset
* @return the effective direction after saturation to +/- STEERING_RELATIV_AMPLITUDE
**/
int16_t Steering_UpdatePwm_Relative_Unsaturated_NOT_TO_BE_USED(int16_t direction) {
	/* Updated duty cycle */
	 int16_t direction_actual=  direction ;
	 uint16_t direction_pwm=  STEERING_RELATIVE_OFFSET + direction_actual;
	TPM_UpdatePwmDutycycle_10000(STEERING_TIMER_BASEADDR,
...
* Returns the effective steering after saturation
**/
uint16_t Steering_UpdatePwm_Absolute(uint16_t direction) {
	/* Updated duty cycle */
	 uint16_t direction_c= direction ;
	 if (direction_c < STEERING_ABSOLUTE_MIN) direction_c= STEERING_ABSOLUTE_MIN;
	 if (direction > STEERING_ABSOLUTE_MAX) direction_c= STEERING_ABSOLUTE_MAX;
	TPM_UpdatePwmDutycycle_10000(STEERING_TIMER_BASEADDR,
...
void Steering_Init(void) {
	...
	Steering_Relative_Zero(); /* Position milieu du servo */
	TPM_StartTimer(STEERING_TIMER_BASEADDR, kTPM_SystemClock);

}

	 // ATTENTION : position centrale = pulse 1.5 ms, signale 50Hz
