...
/* Beware of the duty cycle type => signed to prevent overflow when negative */
 void Motor_UpdatePwm(int16_t left, int16_t left_dir, int16_t right,
		int16_t right_dir) {
	/* Updated duty cycle */
	 //PRINTF("MOTOR %d %d %d %d\n\r", left, left_dir, right, right_dir);
	 uint16_t left_c= left;
	 if (left_c >100) left_c= 0;
	 if (left_c>30) left_c= 30;
	 uint16_t right_c= right;
	 if (right_c > 100) right_c= 0;
	 if (right_c > 30) right_c= 30;
